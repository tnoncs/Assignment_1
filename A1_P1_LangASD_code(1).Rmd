...
```{r}
library(plyr)
LU=plyr::rename(LU, c("SUBJ"="Child.ID", "VISIT"="Visit"))
token=plyr::rename(token, c("SUBJ"="Child.ID", "VISIT"="Visit"))
...
```{r}
demo$Child.ID=gsub("\\.", "", demo$Child.ID) # a pontot minden ID-ben semmire cseréli
LU$Child.ID=gsub("\\.", "", LU$Child.ID)
token$Child.ID=gsub("\\.", "", token$Child.ID)
```
...
```{r}
demo=plyr::rename(demo, c("MullenRaw"="nonverbalIQ", "ExpressiveLangRaw"="verbalIQ"))

demo_sub=demo[,c("Child.ID", "Visit", "Ethnicity", "Diagnosis", "Gender", "Age", "ADOS",  "verbalIQ", "nonverbalIQ")]
LU_sub=LU[,c("Child.ID", "Visit", "MOT_MLU", "MOT_LUstd", "CHI_MLU", "CHI_LUstd")]
token_sub=token[,c("Child.ID", "Visit", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")]
```
...
```{r}
demo_LU=merge(demo_sub, LU_sub)
merged_all=merge(demo_LU, token_sub)

# demo_sub: 372 obs. of 9 variables
# LU_sub: 352 obs. of 6 variables
# token_sub: 352 obs. of 6 variables
# demo_LU: 352 obs. of 13 variables
# merged_all: 352 obs. of 17 variables

check=length(unique(demo_sub$Child.ID))
check1=length(unique(LU_sub$Child.ID))
check2=length(unique(token_sub$Child.ID))
```
...
```{r}
macska=merged_all[, c("Child.ID", "Visit", "nonverbalIQ", "verbalIQ", "ADOS")]
macska_sub=data.frame(macska[macska$Visit==1,])
final=merge(macska_sub, merged_all, by="Child.ID")

final_data=final[, c("Child.ID", "Visit.y", "Ethnicity", "Diagnosis", "Gender", "Age", "ADOS.x", "verbalIQ.x", "nonverbalIQ.x", "MOT_MLU", "MOT_LUstd", "CHI_MLU", "CHI_LUstd", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")]
...
```{r}
children=c(final_data$Child.ID)
vmi=as.numeric(as.factor(children))
final_data$Child.ID=vmi

final_data=plyr::rename(final_data, c("Visit.y"="Visit", "ADOS.x"="ADOS", "verbalIQ.x"="verbalIQ", "nonverbalIQ.x"="nonverbalIQ"))

levels(final_data$Diagnosis)[levels(final_data$Diagnosis)=="A"] <- "ASD"
levels(final_data$Diagnosis)[levels(final_data$Diagnosis)=="B"] <- "TD"

final_data$Gender=factor(final_data$Gender)

levels(final_data$Gender)[levels(final_data$Gender)=="1"] <- "M"
levels(final_data$Gender)[levels(final_data$Gender)=="2"] <- "F"
...
```{r}
# group all visits by children, have a mean of the 6 MLU-s, then get out the names of children for whom it is more than 2.7
library(magrittr)
library(dplyr)

final1 = na.omit(final_data)
...
# 2
visit1=data.frame(final1[final1$Visit==1,])

less= visit1 %>%
  group_by(Child.ID) %>%
  summarise(meanMLU1 = mean(CHI_MLU)) %>%
  filter(meanMLU1 < 1.5)

# 3.
not_comp= final1 %>% 
  group_by(Child.ID) %>% 
  summarise(visits=length(Visit)) %>% 
  filter(visits<6)

# or

missing = na.omit(final_data) %>% 
      group_by(Child.ID) %>% 
      tally() %>% # counts rows
      filter(n < 6)
...
```{r}
# 1
visit6=final_data[final_data$Visit==6,]
most=summarise(group_by(visit6, Child.ID), words=tokens_CHI)
most_words=arrange(most, desc(words)) # the 55th kid, 1294 words

# 2
visit_1=data.frame(final_data[final_data$Visit==1,])
least=summarise(group_by(visit_1, Child.ID), words1=tokens_CHI)
least_words=arrange(least, words1)  # the 57th kid, with 0 words
```
USING SELECT
...
```{r}
# 1
only_asd=filter(final_data, Diagnosis=="ASD")
asd_data=select(only_asd, c(Child.ID, Diagnosis, CHI_MLU, tokens_CHI))
asd_data_multiple=select(only_asd, c(Child.ID, Child.ID, Diagnosis, CHI_MLU, tokens_CHI))
# nothing changes
...
2. Use the summarise function and pipes to add a column in the dataset containing the mean amount of words produced by each trial across all visits. HINT: group by Child.ID 
...
```{r}
average_all = mutate(final_data, GrandAvg = mean(tokens_CHI))

# 2
across_children=average_all %>% 
  group_by(Child.ID) %>% 
  mutate(mean1=mean(tokens_CHI))


ChiAvg_AcrossVisits = average_all %>%
   group_by(Child.ID)%>%
   summarise(ChiAvgWord = mean(tokens_CHI))
  
across_chi=merge(average_all, ChiAvg_AcrossVisits)
